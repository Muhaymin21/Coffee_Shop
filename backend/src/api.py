...
def new_drink(payload):
    ...
    if not data or 'title' not in data or 'recipe' not in data or data['title'] == "":
        return jsonify(
            {"success": False, "error": "Bad request: please make sure to include the title and recipe"}
        ), 400
    if Drink.query.filter_by(title=data['title']).one_or_none() is not None:
        return jsonify(
            {"success": False, "error": f"Coffee {data['title']} is already exist!"}
        ), 422
    try:
...
def update_drink(payload, drink_id):
    ...
    if not data or ('title' not in data and 'recipe' not in data):
        return jsonify(
            {"success": False, "error": "Bad request: please make sure to include the title or recipe"}
        ), 400
